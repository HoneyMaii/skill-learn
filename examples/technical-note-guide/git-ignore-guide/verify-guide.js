...
function staticChecks(html) {
  ...
  const forbidden = [
    { pattern: new RegExp(`\\b${word("TO", "DO")}\\b`, "i"), label: word("TO", "DO") },
    { pattern: new RegExp(`\\b${word("T", "BD")}\\b`, "i"), label: word("T", "BD") },
    { pattern: new RegExp(word("place", "holder"), "i"), label: word("place", "holder") },
    { pattern: new RegExp(`\\.${word("sl", "ide")}\\b`, "i"), label: `.${word("sl", "ide")}` },
    { pattern: new RegExp(word("scroll", "-snap"), "i"), label: word("scroll", "-snap") },
    { pattern: new RegExp(`${word("presentation", "-controls?")}`, "i"), label: word("presentation ", "controls") },
    { pattern: new RegExp(`${word("nav", "-dots?")}`, "i"), label: word("nav ", "dots") },
  ...
  assert(/data-typewriter/.test(html), "Missing Command Typewriter");
  assert(/prompt-template/.test(html), "Missing prompt-template component");
}
...
async function checkCopyButtons(page) {
  ...
  assert(buttons.length >= 6, "Expected at least six copy buttons to test");
